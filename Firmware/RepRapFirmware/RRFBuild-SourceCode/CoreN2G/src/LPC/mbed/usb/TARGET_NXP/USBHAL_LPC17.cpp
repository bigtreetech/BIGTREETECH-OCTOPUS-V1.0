...
USBPhyHw::USBPhyHw(void): events(nullptr)
{
...
//    }
    ...
    //LPC_SC->PCONP |= PCUSB;
    Chip_Clock_EnablePeriphClock(SYSCTL_CLOCK_USB); //enable power and clocking
    ...
    LPC_IOCON->PINSEL[1] &= 0xc3ffffff;
    LPC_IOCON->PINSEL[1] |= 0x14000000;

    // Disconnect USB device
    SIEdisconnect();

    // Configure pin P2.9 to be Connect
    LPC_IOCON->PINSEL[4] &= 0xfffcffff;
    LPC_IOCON->PINSEL[4] |= 0x00040000;
...
bool USBPhyHw::endpoint_add(usb_ep_t endpoint, uint32_t maxPacket, usb_ep_type_t type)
{
    ...
    LPC_USB->EpInd = DESC_TO_PHY(endpoint);
...
void USBPhyHw::endpoint_remove(usb_ep_t endpoint)
{
    ...
    if (LPC_USB->EpIntSt & EP(endpoint)) {
    ...
    if (LPC_USB->DevIntSt & FRAME) {
        ...
        for (uint32_t i = 0; i < sizeof(ISO_EPS) / sizeof(ISO_EPS[0]); i++) {
            ...
            if (opStarted & EP(endpoint)) {
                opStarted &= ~EP(endpoint);
                if (IN_EP(endpoint)) {
                    events->in_packet(endpoint);
                } else {
                    events->out_packet(endpoint);
                }
            }
        }
